...
async def sample_item(
    ...
    Args:
        ...
        create_repository (Callable[[AsyncSession], SampleItemRepository]): 
...
async def sample_item_by_id(
    ...
    This GET endpoint uses the SampleItem repository and the SampleItemGetUseCase
    to fetch a single SampleItem entity, along with its metadata, based on the 
    provided item ID.

    Args:
        entity_id (int): The identifier of the SampleItem to retrieve.
        session_factory (Callable[[], AsyncSession]): An asynchronous session factory 
            for creating database sessions. Injected as a dependency.
        create_repository (Callable[[AsyncSession], SampleItemRepository]): A callable 
            that initializes a SampleItemRepository instance using the provided 
            database session. Injected as a dependency.

    Returns:
        SampleItemWithMeta: A data structure containing the fetched SampleItem entity 
...
async def create_sample_item(
    ...
    This POST endpoint uses the SampleItem repository and the SampleItemCreateUseCase
    to create a new SampleItem entity based on the provided input data.
    ...
    Returns:
        SampleItemRead: The created SampleItem entity serialized into a `SampleItemRead` format.
    """
    async with session_factory() as db_session:
        async with db_session.begin():
            repository = create_repository(db_session)
...
async def update_sample_item(
    ...
    This PUT endpoint leverages the SampleItem repository and the SampleItemUpdateUseCase
    to update an existing SampleItem entity with the new data provided.

    Args:
        entity_id (int): The identifier of the SampleItem to be updated.
        data (SampleItemUpdate): The new data with which to update the SampleItem entity.
        session_factory (Callable[[], AsyncSession]): An asynchronous session factory 
            for creating database sessions. Injected as a dependency.
        create_repository (Callable[[AsyncSession], SampleItemRepository]): A callable 
            that initializes a SampleItemRepository instance using the provided 
    ...
    Returns:
        SampleItemRead: The updated SampleItem entity serialized into a `SampleItemRead` format.
    """
    async with session_factory() as db_session:
        async with db_session.begin():
            repository = create_repository(db_session)
...
async def logical_delete_sample_item(
    ...
    This DELETE endpoint utilizes the SampleItem repository and the SampleItemLogicalDeleteUseCase
    to perform a logical deletion of the given SampleItem entity (e.g., marking it as deleted without 
...
async def physical_delete_sample_item(
    ...
    This DELETE endpoint uses the SampleItem repository and the SampleItemPhysicalDeleteUseCase
    to permanently delete the given SampleItem entity from the database.
